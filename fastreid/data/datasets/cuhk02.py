...
class CUHK02(ImageDataset):
    ...
    def get_data_list(self):
        ...
        for cam_pair in self.cam_pairs:
            cam_pair_dir = osp.join(self.dataset_dir, cam_pair) #//TODO vào từng thư mục P1,...P5
            ...
            if cam_pair == self.test_cam_pair:
                ...
                for impath in impaths1:
                    pid = osp.basename(impath).split('_')[0] #//TODO dùng split để tách '_' chia thành 2 chuỗi và pid là chuỗi đầu tiên 
                    pid = int(pid)
                ...
                pid2label = {
                    pid: label + num_train_pids    #//TODO sau mỗi P thì pid tăng lên = number_train_pids của những P trước
                ...
                for impath in impaths1:
                    ...
                    cam = self.dataset_name + '_' + str(camid)
                    train.append((impath, pid, cam))
                camid += 1
